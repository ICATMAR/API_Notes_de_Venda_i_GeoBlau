...
def pytest_configure(config):
    ...
    Usage:
        ...
        def test_security():
            pass
    """
    config.addinivalue_line("markers", "unit: Tests unitaris")
    config.addinivalue_line("markers", "integration: Tests d'integració")
    config.addinivalue_line("markers", "security: Tests de seguretat")
    config.addinivalue_line("markers", "slow: Tests lents que requereixen molt temps")
...
def multiple_envios(db, darp_user, test_user):
    ...
    darp_envio1 = Envio.objects.create(
        num_envio="DARP_ENV_001", tipo_respuesta=1, usuario_envio=darp_user, ip_origen="192.168.1.10"
    )
    darp_envio2 = Envio.objects.create(
        num_envio="DARP_ENV_002", tipo_respuesta=2, usuario_envio=darp_user, ip_origen="192.168.1.10"
    )

    # Enviament d'un altre usuari
    other_envio = Envio.objects.create(
        num_envio="OTHER_ENV_001", tipo_respuesta=1, usuario_envio=test_user, ip_origen="192.168.1.20"
    )

    return {"darp_envios": [darp_envio1, darp_envio2], "other_envios": [other_envio]}
...
def test_catalog_data(db):
    ...
    """
    from sales_notes.existing_models import Port, Species, Vessel
    ...
    port = Port.objects.create(id=9999, name="Barcelona", code=8, latitude=41.38, longitude=2.17)
