...
class AuditMiddleware(MiddlewareMixin):
    ...
    def log_security_event(self, event_type, description, request, user, 
                          ip_address, severity='MEDIUM', blocked=False):
        """
        ...
        try:
            SecurityEvent.objects.create(
                ...
                request_data_hash=hashlib.sha256(
                    str(request.body).encode()
                ).hexdigest() if request.body else '',
                severity=severity,
                blocked=blocked,
            ...
            logger.warning(
                f"Event de seguretat: {event_type} - {description} "
                f"(IP: {ip_address}, User: {user})"
            )
            
        except Exception as e:
            logger.error(
                f"Error registrant event de seguretat: {str(e)}",
                exc_info=True
            )