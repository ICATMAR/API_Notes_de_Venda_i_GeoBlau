...
class AuditLog(models.Model):
    ...
    old_value = models.JSONField(
        null=True,
        blank=True,
        help_text="Valor anterior (per actualitzacions)"
    )
    new_value = models.JSONField(
        null=True,
        blank=True,
        help_text="Valor nou (per creacions i actualitzacions)"
    )
...
class SecurityEvent(models.Model):
    ...
    blocked = models.BooleanField(
        default=False,
        help_text="Si l'acció va ser bloquejada"
    )
    
    action_taken = models.TextField(
        blank=True,
        help_text="Acció que va prendre el sistema"
    )
    ...
    notified = models.BooleanField(
        default=False,
        help_text="Si s'ha notificat als responsables"
    )
...
class DataValidationError(models.Model):
    ...
    field_name = models.CharField(
        max_length=100,
        db_index=True,
        help_text="Nom del camp amb error"
    )
    
    error_type = models.CharField(
        max_length=50,
        db_index=True,
        help_text="Tipus d'error (ex: required, format, range)"
    )
    
    error_message = models.TextField(
        help_text="Missatge d'error detallat"
    )
    
    # Valor que va causar l'error
    invalid_value = models.TextField(
        blank=True,
        help_text="Valor que no va passar la validació"
    )
    
    expected_format = models.TextField(
        blank=True,
        help_text="Format esperat"
    )
    ...
    json_path = models.CharField(
        max_length=500,
        blank=True,
        help_text="Path JSON on es troba l'error"
    )
