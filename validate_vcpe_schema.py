            ...
            if isinstance(obj, dict):
                ...
                for pattern in suspicious_patterns:
                    if pattern.lower() in obj.lower():
                        errors.append(f"ALERTA SEGURETAT: Patró sospitós '{pattern}' detectat a {path}")
...
def run_security_tests():
    ...
    sql_injection_payload = {"NumEnvio": "ENV001'; DROP TABLE ventas; --", "TipoRespuesta": 1}
    ...
    xss_payload = {
        ...
        "EstablecimientosVenta": {
            "EstablecimientoVenta": [
                {"NumIdentificacionEstablec": "<script>alert('xss')</script>", "Ventas": {"VentasUnidadProductiva": []}}
            ]
        },
    }
    ...
    path_traversal_payload = {
        ...
        "EstablecimientosVenta": {
            "EstablecimientoVenta": [
                {"NumIdentificacionEstablec": "../../../etc/passwd", "Ventas": {"VentasUnidadProductiva": []}}
            ]
        },
    }
    ...
    dos_payload = {
        ...
        "EstablecimientosVenta": {
            "EstablecimientoVenta": [
                {"NumIdentificacionEstablec": f"EST{i:06d}", "Ventas": {"VentasUnidadProductiva": []}}
