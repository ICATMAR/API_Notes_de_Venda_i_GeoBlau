# Generated migration to update trigger target schema to landings and enhance parsing logic
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("sales_notes", "0005_alter_envio_usuario_envio"),
    ]

    operations = [
        migrations.RunSQL(
            sql="""
            -- 1. Assegurar que la columna EnvioId existeix a la taula de landings si no hi era
            ALTER TABLE landings.vcpe_auto_download ADD COLUMN IF NOT EXISTS "EnvioId" uuid NULL;

            -- 2. Re-crear la funció del Trigger apuntant a landings.vcpe_auto_download
            --    i millorant el parseig i normalització
                    ...
                    LOOP
                        ...
                        IF estab_element -> 'Ventas' -> 'VentasUnidadProductiva' IS NOT NULL THEN
                            ...
                            LOOP
                                ...
                                -- Determinar tipus de vaixell o persona/mariscador
                                ...
                                -- Inicialitzar variables de vaixell per a cada iteració
                                ...
                                -- Cercar vaixell a la taula vessel
                                ...
                                -- 1. Intentar cercar per portAL5 o LugarDescarga
                                ...
                                -- 2. Mapeig alternatiu per NIFs segons NumIdentificacionEstablec
                                ...
                                -- 3. Cercar per nom d'establiment amb neteja i accent-insensitive
                                ...
                                IF especies_json IS NOT NULL THEN
                                    ...
                                    LOOP
                                        ...
                                        INSERT INTO landings.vcpe_auto_download (
                                            ...
                                            v_kg,
                                            (v_kg * v_eur), -- Amount com a total (pes * preu)
                                            v_port_id,
                                            v_vessel_id,
                                            v_specie_id,
                                            v_track_code,
            ...
            """,
            reverse_sql="""
            -- Revertir i apuntar de nou a public.vcpe_auto_download
            CREATE OR REPLACE FUNCTION parse_and_insert_sales_notes()
            RETURNS TRIGGER AS $$
            DECLARE
                estab_element jsonb; venta_element jsonb; especie_element jsonb;
