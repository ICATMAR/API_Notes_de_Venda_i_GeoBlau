    ...
    def validate(self, data):
        ...
        if data.get("fecha_captura_fin"):
            if data["fecha_captura_fin"] < data["fecha_captura_ini"]:
                raise serializers.ValidationError("La data de fi de captura no pot ser anterior a la d'inici")
        return data
...
class EspecieSerializer(serializers.ModelSerializer):
    ...
    def validate_especie_al3(self, value):
        ...
        if not value.isalpha() or len(value) != 3:
            raise serializers.ValidationError("El codi d'espècie ha de ser 3 lletres (format FAO AL3)")
        return value.upper()

    def validate_cantidad(self, value):
        """Validar que la quantitat sigui positiva"""
        if value <= 0:
            raise serializers.ValidationError("La quantitat ha de ser superior a 0")
        return value

    def validate_precio(self, value):
        """Validar que el preu sigui positiu o zero"""
        if value < 0:
            raise serializers.ValidationError("El preu no pot ser negatiu")
        return value
    ...
    def validate(self, data):
        ...
        if data.get("tipo_retirada") and data.get("tipo_retirada") != 1:
            if not data.get("cod_destino_retirado"):
                raise serializers.ValidationError("Si hi ha retirada, cal especificar el codi de destí")
        ...
        if not data.get("es_talla_reglamentaria", True):
            if not data.get("observaciones"):
                logger.warning(f"Espècie {data.get('especie_al3')} amb talla no reglamentària sense observacions")
...
class BuqueSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Buque
        fields = ["codigo_buque", "puerto_al5", "armador", "capitan", "fecha_regreso_puerto", "cod_marea"]

    def validate_puerto_al5(self, value):
        """Validar format del codi de port"""
        if value and len(value) != 5:
            raise serializers.ValidationError("El codi de port ha de tenir 5 caràcters (2 lletres + 3 alfanumèrics)")
...
class GranjaSerializer(serializers.ModelSerializer):
    ...
    def validate_codigo_rega(self, value):
        ...
        if len(value) > 14:
            raise serializers.ValidationError("El codi REGA no pot superar 14 caràcters")
        return value
...
class PersonaFisicaJuridicaSerializer(serializers.ModelSerializer):
    ...
    def validate_nif_persona(self, value):
        ...
        if len(value) > 17:
            raise serializers.ValidationError("El NIF no pot superar 17 caràcters")
        return value.upper()
    ...
    def validate(self, data):
        ...
        if metodo in [1, 4]:  # Pesca extractiva o aqüicultura aigües interiors
            ...
            if not (data.get("buque") or data.get("persona")):
                raise serializers.ValidationError("Per mètode de producció 1 o 4, cal especificar buque o persona")
        elif metodo == 2:  # Aqüicultura marina
            if not data.get("granja"):
                raise serializers.ValidationError("Per mètode de producció 2, cal especificar dades de granja")
        elif metodo == 3:  # Pesca extractiva aigües interiors
            if not data.get("persona"):
                raise serializers.ValidationError("Per mètode de producció 3, cal especificar dades de persona")

        # Validar que només hi hagi un tipus d'unitat
        tipus_presents = sum([bool(data.get("buque")), bool(data.get("granja")), bool(data.get("persona"))])
...
class EstablecimientoVentaSerializer(serializers.ModelSerializer):
    ...
    ventas_unidad_productiva = UnidadProductivaSerializer(many=True, source="unidades_productivas", required=False)
    ...
    def validate_num_identificacion_establec(self, value):
        ...
        if not value or len(value) == 0:
            raise serializers.ValidationError("El número d'identificació de l'establiment és obligatori")
        return value
...
class EnvioSerializer(serializers.ModelSerializer):
    ...
    establecimientos_venta = EstablecimientoVentaSerializer(many=True, source="establecimientos", required=False)
    ...
    def validate_num_envio(self, value):
        ...
        if Envio.objects.filter(num_envio=value).exists():
            raise serializers.ValidationError(f"Ja existeix un enviament amb el número {value}")
        return value

    def validate_tipo_respuesta(self, value):
        """Validar tipus de resposta"""
        if value not in [1, 2, 3]:
            raise serializers.ValidationError("El tipus de resposta ha de ser 1 (Completa), 2 (Mitjana) o 3 (Reduïda)")
        return value
    ...
    def validate(self, data):
        ...
        if not establecimientos:
            logger.warning(f"Enviament {data.get('num_envio')} sense establiments de venda")
        ...
        if total_especies == 0:
            raise serializers.ValidationError("L'enviament ha de contenir almenys una espècie")
    ...
    def create(self, validated_data):
        ...
        # Crear l'enviament
        envio = Envio.objects.create(usuario_envio=user, ip_origen=ip_address, **validated_data)

        logger.info(f"Creant enviament {envio.num_envio} per usuari {user.username}")
        ...
        try:
            for establecimiento_data in establecimientos_data:
                ...
                establecimiento = EstablecimientoVenta.objects.create(envio=envio, **establecimiento_data)
            ...
            logger.info(
                f"Enviament {envio.num_envio} creat correctament amb " f"{envio.establecimientos.count()} establiments"
            )

        except Exception as e:
            logger.error(f"Error creant enviament {envio.num_envio}: {str(e)}", exc_info=True)
            # Afegir error a l'enviament
            envio.errores.append({"error": str(e), "timestamp": datetime.now().isoformat()})
        ...
        for esp in especies:
            ...
            if fechas_captura:
                converted_esp["fechas_captura"] = [
                    {"fecha_captura_ini": fc.get("FechaCapturaIni"), "fecha_captura_fin": fc.get("FechaCapturaFin")}
...
class EnvioStatusSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Envio
        fields = ["num_envio", "fecha_recepcion", "procesado", "fecha_procesado", "validado", "errores"]
        read_only_fields = fields
