...
class Envio(TimeStampedModel):
    ...
    num_envio = models.CharField(
        max_length=50, 
        unique=True, 
        db_index=True,
        help_text="Número únic d'enviament"
    )
...
class EstablecimientoVenta(TimeStampedModel):
    ...
    num_identificacion_establec = models.CharField(
        max_length=50,
        db_index=True,
        help_text="Número d'identificació de l'establiment"
    )
...
class UnidadProductiva(TimeStampedModel):
    ...
    metodo_produccion = models.IntegerField(
        choices=METODO_PRODUCCION_CHOICES,
        db_index=True
    )
...
class Buque(models.Model):
    ...
    puerto_al5 = models.CharField(
        max_length=5,
        validators=[puerto_al5_validator],
        blank=True,
        null=True,
...
class Granja(models.Model):
    ...
    codigo_rega = models.CharField(
        max_length=14,
        db_index=True,
        help_text="Codi REGA de la granja"
    )
    
    lugar_descarga = models.CharField(
        max_length=250,
        blank=True,
        help_text="Lloc de descàrrega"
    )
    
    fecha_produccion = models.DateTimeField(
        help_text="Data de producció"
    )
...
class PersonaFisicaJuridica(models.Model):
    ...
    nif_persona = models.CharField(
        max_length=17,
        db_index=True,
        help_text="NIF/NIE/CIF de la persona"
    )
...
class Especie(TimeStampedModel):
    ...
    """
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    unidad_productiva = models.ForeignKey(
        'UnidadProductiva',
        on_delete=models.CASCADE,
    ...
    num_doc_venta = models.CharField(
        max_length=21,
        db_index=True,
        help_text="Número del document de venda"
    )
    ...
    especie_al3 = models.CharField(
        max_length=3,
        validators=[codigo_al3_validator],
        db_index=True,
    ...
    ccaa = models.IntegerField(
        null=True,
        blank=True,
        help_text="Codi Comunitat Autònoma"
    )
    ...
    masa_agua = models.CharField(
        max_length=100,
        blank=True,
        help_text="Massa d'aigua on es va capturar"
    )
    
    # Art de pesca
    arte_al3 = models.CharField(
        max_length=3,
        blank=True,
        db_index=True,
        help_text="Codi d'art de pesca (FAO)"
    )
    
    otro_arte = models.CharField(
        max_length=100,
        blank=True,
        help_text="Descripció d'altres arts no estandarditzats"
    )
    
    # Presentació i estat del producte
    cod_especie_conservacion = models.CharField(
        max_length=3,
        blank=True,
        help_text="Codi estat de conservació"
    )
    
    cod_especie_presentacion = models.CharField(
        max_length=5,
        blank=True,
        help_text="Codi presentació del producte"
    )
    
    presentacion_oth = models.CharField(
        max_length=50,
        blank=True,
        help_text="Altres presentacions"
    )
    
    id_presentacion_oth = models.IntegerField(null=True, blank=True)
    
    cod_especie_frescura = models.CharField(
        max_length=2,
        blank=True,
        help_text="Codi frescura"
    )
    
    cod_especie_calibre = models.CharField(
        max_length=15,
        blank=True,
        help_text="Codi calibre"
    )
    
    # Data de venda
    fecha_venta = models.DateTimeField(
        db_index=True,
        help_text="Data i hora de venda"
    )
    
    lote = models.CharField(
        max_length=50,
        blank=True,
        help_text="Número de lot"
    )
    
    cod_contrato_alim = models.CharField(
        max_length=50,
        blank=True,
        help_text="Codi contracte alimentari"
    )
    
    # Documents associats
    num_doc_transporte = models.CharField(
        max_length=50,
        blank=True,
        help_text="Número document de transport"
    )
    
    num_declaracion_recog = models.CharField(
        max_length=21,
        blank=True,
        help_text="Número declaració recollida"
    )
    
    certificado_de_origen = models.BooleanField(
        default=False,
        help_text="Té certificat d'origen"
    )
    ...
    tipo_cif_nif_vendedor = models.IntegerField(
        choices=TIPO_NIF_CHOICES,
        help_text="Tipus document identificació venedor"
    )
    
    nif_vendedor = models.CharField(
        max_length=17,
        db_index=True,
        help_text="NIF/CIF del venedor"
    )
    
    nombre_vendedor = models.CharField(
        max_length=100,
        help_text="Nom del venedor"
    )
    
    direccion_vendedor = models.CharField(
        max_length=100,
        blank=True,
        help_text="Direcció del venedor"
    )
    
    # Comprador
    nif_comprador = models.CharField(
        max_length=17,
        db_index=True,
        help_text="NIF/CIF del comprador"
    )
    ...
    pais_comprador_validator = RegexValidator(
        regex=r'^[A-Za-z]{3}$',
        message='Codi país invàlid (ISO-3, 3 lletres)'
    )
    pais_comprador = models.CharField(
        max_length=3,
        validators=[pais_comprador_validator],
        blank=True,
    ...
    nombre_comprador = models.CharField(
        max_length=100,
        help_text="Nom del comprador"
    )
    
    direccion_comprador = models.CharField(
        max_length=100,
        blank=True,
        help_text="Direcció del comprador"
    )
    ...
    cod_moneda_validator = RegexValidator(
        regex=r'^[A-Za-z]{3}$',
        message='Codi moneda invàlid (ISO-4217, 3 lletres)'
    )
    cod_moneda = models.CharField(
        max_length=3,
        default='EUR',
        validators=[cod_moneda_validator],
    ...
    es_talla_reglamentaria = models.BooleanField(
        default=True,
        help_text="És talla reglamentària"
    )
    ...
    tipo_retirada = models.IntegerField(
        choices=TIPO_RETIRADA_CHOICES,
        null=True,
        blank=True,
        help_text="Tipus de retirada"
    )
    
    cod_destino_retirado = models.CharField(
        max_length=50,
        blank=True,
        help_text="Codi destí del producte retirat"
    )
    
    # Emmagatzematge i observacions
    lugar_almacenamiento = models.CharField(
        max_length=100,
        blank=True,
        help_text="Lloc d'emmagatzematge"
    )
    
    observaciones = models.TextField(
        blank=True,
        help_text="Observacions addicionals"
    )
    
    num_acta_inspeccion = models.CharField(
        max_length=20,
        blank=True,
...
class FechaCaptura(models.Model):
    ...
    fecha_captura_ini = models.DateTimeField(
        help_text="Data i hora inici captura"
    )
    
    fecha_captura_fin = models.DateTimeField(
        null=True,
        blank=True,
        help_text="Data i hora fi captura (opcional)"
    )
