...
class UserRegistrationSerializer(serializers.ModelSerializer):
    ...
    password_confirm = serializers.CharField(
        write_only=True, required=True, style={"input_type": "password"}, help_text=_("Must match password field")
    )
...
class LoginSerializer(serializers.Serializer):
    ...
    username = serializers.CharField(required=True, help_text=_("Username for authentication"))

    password = serializers.CharField(
        required=True, style={"input_type": "password"}, write_only=True, help_text=_("User password")
    )
...
class TokenResponseSerializer(serializers.Serializer):
    ...
    access_token = serializers.CharField(read_only=True, help_text=_("JWT access token for API authentication"))

    refresh_token = serializers.CharField(
        read_only=True, help_text=_("JWT refresh token for obtaining new access tokens")
    )

    token_type = serializers.CharField(read_only=True, default="Bearer", help_text=_("Token type (Bearer)"))

    expires_in = serializers.IntegerField(read_only=True, help_text=_("Access token expiration time in seconds"))

    user = UserSerializer(read_only=True, help_text=_("Authenticated user information"))
...
class TokenRefreshSerializer(serializers.Serializer):
    ...
    refresh_token = serializers.CharField(required=True, help_text=_("Valid refresh token"))
...
class PasswordChangeSerializer(serializers.Serializer):
    ...
    old_password = serializers.CharField(
        required=True, write_only=True, style={"input_type": "password"}, help_text=_("Current password")
    )
    ...
    new_password_confirm = serializers.CharField(
        required=True, write_only=True, style={"input_type": "password"}, help_text=_("Confirm new password")
    )
    ...
    def validate(self, attrs):
        ...
        if attrs["new_password"] != attrs["new_password_confirm"]:
            raise serializers.ValidationError({"new_password": _("New password fields do not match")})
